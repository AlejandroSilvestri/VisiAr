...
if(altoImagen > alturaObjetivo):
    ...
    print('Resolución final:', tamanoObjetivo)
...
cv.createTrackbar('umbral', 'video' , 0, 255, umbralCannyTrackbar)
...
def calcularRoi(H_ = None):
    ...
    xProyectado = medio*(altoImagen-tope)//(altoImagen-horizonte)
...
while(True):
    ...
    # Fast Line Detector
    # lineas shape (n,1,4), dtype float32
    limiteSuperior = (altoImagen-horizonte)//2
    lineas = detector.detect(imGris[limiteSuperior:])
    lineas = lineas + np.array([0,limiteSuperior,0,limiteSuperior], np.float32)
    limite = (limiteSuperior+altoImagen)/2
    lineasFiltradas = lineas[np.where((lineas[:,0,1]>limite) | (lineas[:,0,3]>limite))]
    #imAnotada = detector.drawSegments(im, lineas)
    imAnotada = detector.drawSegments(im, lineasFiltradas, linecolor=(32,128,128), linethickness=2)
